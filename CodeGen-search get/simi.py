...
def my_ranker(input_sentence, list_of_sentences, is_list):
    best_score = 0.0
    best_sentence = ""
    index = -1
    for i, sentence in enumerate(list_of_sentences):
        # simi_score = simi(input_sentence, sentence, is_list)
        simi_score = simi(input_sentence, sentence, is_list)    
        if simi_score > best_score and simi_score != 1:
            best_score = simi_score
            best_sentence = sentence
            index = i
    return best_score, best_sentence,index
...
if __name__ == "__main__":
    ...
    query = "Darkscale Healer NAME_END 4 ATK_END 5 DEF_END 5 COST_END -1 DUR_END Minion TYPE_END NEUTRAL PLAYER_CLS_END NIL RACE_END COMMON RARITY_END < b > battlecry : < /b > restore 2 health to all friendly characters ."
    score, sentence, index = my_ranker(query, sent_list, False)
    print("ranker")
    print(score)
    print(sentence)
    print(index)
