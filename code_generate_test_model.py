...
class code_gen_model:
    ...
    def multiheadattention_QKV_2(self, Query, Keys, Values, mask, flag=False, antimask=""):
        ...
        W_o = tf.layers.dense(concat_head, d, name="qkv2head",
                              use_bias=False)  # self.weight_variable(shape=[int(concat_head.shape[2]), d])
    ...
    def multiheadattention(self, H, mask, k, flag=False, antimask="", use_posi_att=False, posi_embedding=""):
        ...
        for i in range(k):
            W_q = tf.layers.dense(H, d // k, name="headq" + str(i),
                                  use_bias=False)  # self.weight_variable(shape=[d, d // k])
            W_k = tf.layers.dense(H, d // k, name="headk" + str(i),
                                  use_bias=False)  # self.weight_variable(shape=[d, d // k])
            W_v = tf.layers.dense(H, d // k, name="headv" + str(i),
                                  use_bias=False)  # self.weight_variable(shape=[d, d // k])
            if use_posi_att:
                # if not flag:
                ...
                list_concat.append(
                    self.headattention_position(W_q, W_k, W_v, antimask, posi, posi_k, posi_v, flag, mask))
            else:
                list_concat.append(self.headattention(W_q, W_k, W_v, mask, flag, antimask))
        concat_head = tf.concat(list_concat, -1)
        W_o = tf.layers.dense(concat_head, d, name="head",
                              use_bias=False)  # self.weight_variable(shape=[int(concat_head.shape[2]), d])
        return W_o  # self.mul(concat_head, W_o)
    ...
    def multiheadattention_QKV_Copy(self, p, Query, Keys, Values, mask):
        ...
        heads = k = 1
        for i in range(heads):
            W_q = tf.layers.dense(Query, d // k, name="qkv_headq" + str(i),
                                  use_bias=False)  # self.weight_variable(shape=[d, d // k])
            W_k = tf.layers.dense(Keys, d // k, name="qkv_headk" + str(i),
                                  use_bias=False)  # self.weight_variable(shape=[d, d // k])
    ...
    def multiheadattention_QKV(self, Query, Keys, Values, mask, flag=False, antimask="", use_posi_att=False,
                               posi_embedding=""):
        m = int(Values.shape[1])
        d = int(Values.shape[2])
        list_concat = []
        ...
        heads = 8
        for i in range(heads):
            W_q = tf.layers.dense(Query, d // heads, name="qkv_headq" + str(i),
                                  use_bias=False)  # self.weight_variable(shape=[d, d // k])
            W_k = tf.layers.dense(Keys, d // heads, name="qkv_headk" + str(i),
                                  use_bias=False)  # self.weight_variable(shape=[d, d // k])
            W_v = tf.layers.dense(Values, d // heads, name="qkv_headv" + str(i),
                                  use_bias=False)  # self.weight_variable(shape=[d, d // k])
            list_concat.append(self.headattention(W_q, W_k, W_v, mask, flag, antimask))
        concat_head = tf.concat(list_concat, -1)
        W_o = tf.layers.dense(concat_head, d, name="qkv_head",
                              use_bias=False)  # self.weight_variable(shape=[int(concat_head.shape[2]), d])
        return W_o  # self.mul(concat_head, W_o)
    ...
    def sepconv(self, state, size, mask):
        state = self.drop(tf.layers.separable_conv1d(tf.expand_dims(mask, -1) * self.drop(
            tf.layers.separable_conv1d(state, size, 3, activation=self.gelu, padding="SAME", name="conv")), size, 3,
                                                     padding="SAME", name="dense_2") + state)
        return state
    ...
    def test_info_reader(self, state, embedding_size, step, halting_probability, remainders, n_updates, previous_state,
                  em_char, mask, mask1):
        size = embedding_size
        i = step
        state_shape_static = state.get_shape()
        state += self.get_timing_signal_1d(tf.shape(state)[1], tf.shape(state)[2], start_index=0) \
                 + self.get_timing_signal_1d(tf.shape(state)[1], tf.shape(state)[2],
                                             start_index=step)  # self.position_embedding(state, i)
        ...
        with tf.variable_scope("Char_Att", reuse=None):
            state = self.layer_norm(
                self.drop(self.multiheadattention_QKV_2(state, state, em_char, mask, False, "") + state), "norm2")
            # self.probe = state
            # state += em_char
    ...
    def nl_reader(self, state, embedding_size, step, halting_probability, remainders, n_updates, previous_state,
                  em_char, mask, mask1):
        size = embedding_size
        i = step
        state_shape_static = state.get_shape()
    ...
    def ast_reader(self, state, embedding_size, step, halting_probability,
                   remainders, n_updates, previous_state, Decoder1, nl_conv, state_ast,code_conv,test_info_conv):
        size = embedding_size
        i = step
        state_shape_static = state.get_shape()
        ...
        Decoder += self.em_depth + self.get_timing_signal_1d(tf.shape(state)[1], tf.shape(state)[2],
                                                             start_index=0) + self.get_timing_signal_1d(
            tf.shape(state)[1], tf.shape(state)[2], start_index=step)  # self.position_embedding(state, i)
        Decoder = self.layer_norm(self.drop(
            self.multiheadattention_QKV(Decoder, Decoder, Decoder, self.mask_rule, True, self.antimask) + Decoder),
                                  "norm1")
        with tf.variable_scope("TP_R", reuse=None):
            Decoder = self.layer_norm(self.drop(
                self.multiheadattention_QKV_2(Decoder, Decoder, em_Rule_Type, self.mask_rule, False,
                                              self.antimask) + Decoder), "norm3")

        with tf.variable_scope("NL_ATT", reuse=None):
            Decoder = self.layer_norm(
                self.drop(Decoder + self.multiheadattention_QKV(Decoder, nl_conv, nl_conv, self.mask_nl)))

        # with tf.variable_scope("my_code_ATT", reuse=None):
        #     Decoder = self.layer_norm(
        #         self.drop(Decoder + self.multiheadattention_QKV(Decoder, code_conv, code_conv, self.mask_nl)))
        # with tf.variable_scope("my_test_info_ATT", reuse=None):
        #     Decoder = self.layer_norm(
        #         self.drop(Decoder + self.multiheadattention_QKV(Decoder, test_info_conv, test_info_conv, self.mask_nl)))

        Decoder = self.drop(self.sepconv_A(Decoder, self.tree_A, 3))
        state = Decoder = self.layer_norm(Decoder, "norm4")
        new_state = previous_state
        step += 1
        return (state, embedding_size, step, halting_probability, remainders, n_updates, new_state, Decoder1, nl_conv,
                state_ast,code_conv,test_info_conv)
    def code_reader(self, state,  step, halting_probability, remainders, n_updates,
                    previous_state, Decoder1, nl_conv,test_info_conv, state_ast):
        # self.code_reader(state, self.embedding_size, step, halting_probability, remainders, n_updates,
        #                  previous_state, em_Rule_Type, nl_conv, test_info_conv, em_code, mask_code)
        ...
        #                                                      start_index=0) + self.get_timing_signal_1d(
        #     tf.shape(state)[1], tf.shape(state)[2], start_index=step)  # self.position_embedding(state, i)
        Decoder += self.get_timing_signal_1d(tf.shape(state)[1], tf.shape(state)[2],
                                                             start_index=0) + self.get_timing_signal_1d(
            tf.shape(state)[1], tf.shape(state)[2], start_index=step)  # self.position_embedding(state, i)
        Decoder = self.layer_norm(self.drop(
            self.multiheadattention_QKV(Decoder, Decoder, Decoder, self.mask_code) + Decoder),
                                  "my_code_norm")
        # with tf.variable_scope("TP_R", reuse=None):
        #     Decoder = self.layer_norm(self.drop(
        #         self.multiheadattention_QKV_2(Decoder, Decoder, em_Rule_Type, self.mask_rule, False,
        ...
        with tf.variable_scope("code_NL_ATT", reuse=None):
            Decoder = self.layer_norm(
                self.drop(Decoder + self.multiheadattention_QKV(Decoder, test_info_conv, test_info_conv, self.mask_test_info)))
        # with tf.variable_scope("code_test_info_ATT", reuse=None):
        #     Decoder = self.layer_norm(
        #         self.drop(Decoder + self.multiheadattention_QKV(Decoder, nl_conv, nl_conv, self.mask_nl)))

        # Decoder = self.drop(self.sepconv_A(Decoder, self.tree_A, 3))
        Decoder *= tf.expand_dims(self.mask_code,-1)

        Decoder = self.drop(self.sepconv(Decoder, self.embedding_size, self.mask_code))
        Decoder = self.layer_norm(Decoder, "norm4")
        new_state = previous_state
        step += 1
        return (Decoder, step, halting_probability, remainders, n_updates, new_state, Decoder1,  nl_conv,test_info_conv,
                state_ast)
    def query_decoder(self, state, embedding_size, step, halting_probability, remainders, n_updates, previous_state,
                      ast_p, nl_conv, tree_path):
        size = embedding_size
        i = step
        state_shape_static = state.get_shape()
        Decoder = state
        ...
        Decoder = self.layer_norm(self.drop(
            self.multiheadattention_QKV(Decoder, ast_p, ast_p, self.mask_rule, True, self.antimask) + Decoder), "norm1")
        with tf.variable_scope("TP_R", reuse=None):
            Decoder = self.layer_norm(self.drop(
                self.multiheadattention_QKV(Decoder, nl_conv, nl_conv, self.mask_nl, False, self.antimask) + Decoder),
                                      "norm3")
        Decoder = self.drop(
            tf.layers.dense(self.drop(self.gelu(tf.layers.dense(Decoder, self.embedding_size * 4, name="decode2"))),
                            self.embedding_size, name="decode1") + Decoder)
        state = Decoder = self.layer_norm(Decoder, "norm4")
        step += 1
        new_state = previous_state
        return (
        state, embedding_size, step, halting_probability, remainders, n_updates, new_state, ast_p, nl_conv, tree_path)

    def my_query_decoder(self, state, embedding_size, step, halting_probability, remainders, n_updates, previous_state,
                      ast_p, nl_conv, tree_path,test_info_conv,code_conv):
        size = embedding_size
        i = step
        state_shape_static = state.get_shape()
        Decoder = state

        # Decoder = self.layer_norm(self.drop(self.multiheadattention_QKV_2(Decoder, Decoder, Decoder, self.mask_rule, True, self.antimask) + Decoder), "my_norm1")
        # Decoder = self.layer_norm(self.drop(
        #     self.multiheadattention_QKV(Decoder, Decoder, Decoder, self.mask_rule, True) + Decoder),
        #     "my_code_norm")

        with tf.variable_scope("AST_p", reuse=tf.AUTO_REUSE):
            Decoder = self.layer_norm(self.drop(
                self.multiheadattention_QKV(Decoder, ast_p, ast_p, self.mask_rule, True, self.antimask) + Decoder), "norm1")

        with tf.variable_scope("TP_R", reuse=tf.AUTO_REUSE):
            Decoder = self.layer_norm(self.drop(
                self.multiheadattention_QKV(Decoder, nl_conv, nl_conv, self.mask_nl, False, self.antimask) + Decoder),"norm3")

        with tf.variable_scope("code_R", reuse=None):
            Decoder = self.layer_norm(self.drop(
                self.multiheadattention_QKV(Decoder, code_conv, code_conv, self.mask_code, False,
                                            self.antimask) + Decoder),
                "my_code_norm")
        with tf.variable_scope("test_info_R", reuse=None):
            Decoder = self.layer_norm(self.drop(
                self.multiheadattention_QKV(Decoder, test_info_conv, test_info_conv, self.mask_test_info, False,
                                            self.antimask) + Decoder),
                "my_test_info_norm")
        with tf.variable_scope("AST_p", reuse=tf.AUTO_REUSE):
            Decoder = self.layer_norm(self.drop(
        ...
        # with tf.variable_scope("test_info_R", reuse=None):
        #     Decoder = self.layer_norm(self.drop(
        #         self.multiheadattention_QKV(Decoder, test_info_conv,test_info_conv , self.mask_test_info, False, self.antimask) + Decoder),
        #                               "norm3")

        # with tf.variable_scope("code_R", reuse=None):
        #     Decoder = self.layer_norm(self.drop(
        #         self.multiheadattention_QKV(Decoder, code_conv,code_conv , self.mask_code, False, self.antimask) + Decoder),
        #                               "norm3")
        Decoder = self.drop(
            tf.layers.dense(self.drop(self.gelu(tf.layers.dense(Decoder, self.embedding_size * 4, name="decode2"))),
                            self.embedding_size, name="decode1") + Decoder)
        ...
        new_state = previous_state
        return (
        state, embedding_size, step, halting_probability, remainders, n_updates, new_state, ast_p, nl_conv, tree_path,test_info_conv,code_conv)


    def my_query_decoder_2(self, state, embedding_size, step, halting_probability, remainders, n_updates, previous_state,
                      ast_p, nl_conv, tree_path,test_info_conv,code_conv):
        size = embedding_size
        i = step
        state_shape_static = state.get_shape()
        ...
        step += 1
        new_state = previous_state
        return (Decoder, embedding_size, step, halting_probability, remainders, n_updates, new_state, ast_p, nl_conv, tree_path,test_info_conv,code_conv)
    ...
    def transf(self, em_NL, mask, layers, position=False, size=None, em_char=""):
        ...
        for i in range(int(self.max_steps)):
            with tf.variable_scope("NL_CONV" + str(i), reuse=None):
                (state, size, step, halting_probability, remainders, n_updates,
                 previous_state, em_char, mask, mask) \
                    = self.nl_reader(state, size, step, halting_probability, remainders, n_updates,
                                     previous_state, em_char, mask, mask)
        # self.probe = state
        return state

    def test_info_transf(self, em_NL, mask, layers, position=False, size=None, em_char=""):
        if size is None:
            size = int(em_NL.shape[2])
        state = em_NL
        state_slice = slice(0, 2)
        ...
        for i in range(int(self.max_steps)):
            with tf.variable_scope("test_info_CONV" + str(i), reuse=None):
                (state, size, step, halting_probability, remainders, n_updates,
                 previous_state, em_char, mask, mask) \
                    = self.test_info_reader(state, size, step, halting_probability, remainders, n_updates,
                                     previous_state, em_char, mask, mask)
        # self.probe = state
        return state
    ...
    def __init__(self, classnum, embedding_size, conv_layernum, conv_layersize, rnn_layernum,
        ...
        self.embedding = tf.get_variable("embedding", [NL_vocabu_size, embedding_size],
                                         dtype=tf.float32,
                                         initializer=tf.random_uniform_initializer(-math.sqrt(3), math.sqrt(3)))
        self.char_embedding = tf.get_variable("char_embedding", [Char_vocabu_size, embedding_size],
                                              dtype=tf.float32,
                                              initializer=tf.random_uniform_initializer(-math.sqrt(3), math.sqrt(3)))
        self.Tree_embedding = tf.get_variable("Tree_embedding", [Tree_vocabu_size, embedding_size],
                                              dtype=tf.float32,
                                              initializer=tf.random_uniform_initializer(-math.sqrt(3), math.sqrt(3)))
        self.Rule_embedding = tf.get_variable("Rule_embedding", [classnum + 10, embedding_size],
                                              dtype=tf.float32,
                                              initializer=tf.random_uniform_initializer(-math.sqrt(3), math.sqrt(3)))
        channels = embedding_size
        self.depth_embedding = tf.get_variable("Depth_embedding", [40, embedding_size], dtype=tf.float32,
                                               initializer=tf.truncated_normal_initializer(stddev=channels ** -0.5)) * (
                                           channels ** 0.5)
        ...
        #my input------------- test_info ------------start -------------
        self.input_test_info = tf.placeholder(tf.int32, shape=[None, NL_len])
        self.mask_test_info = self.weights_nonzero(self.input_test_info)
        self.test_info_embedding = tf.get_variable("test_info_embedding", [NL_vocabu_size, embedding_size],
                                         dtype=tf.float32,
                                         initializer=tf.random_uniform_initializer(-math.sqrt(3), math.sqrt(3)))
        em_test_info  = tf.nn.embedding_lookup(self.test_info_embedding, self.input_test_info)
        self.input_test_info_Char = tf.placeholder(tf.int32, shape=[None, NL_len, 10])
        self.input_test_info_char_embedding = tf.get_variable("input_test_info_char_embedding", [Char_vocabu_size, embedding_size],
                                              dtype=tf.float32,
                                              initializer=tf.random_uniform_initializer(-math.sqrt(3), math.sqrt(3)))
        em_input_test_info_char = tf.nn.embedding_lookup(self.input_test_info_char_embedding, self.input_test_info_Char)
        with tf.variable_scope("test_char_embedding", reuse=None):
            em_input_test_info_char_conv = self.drop(tf.layers.conv2d(em_input_test_info_char, embedding_size, [1, 10], name="test_char_ebd"))
            em_input_test_info_char = tf.reduce_max(em_input_test_info_char_conv, reduction_indices=[-2])
            em_input_test_info_char = self.layer_norm(em_input_test_info_char)
        # my input------------- test_info ------------end ---------------

        # my input------------- input_code ------------start -------------
        self.input_code = tf.placeholder(tf.int32, shape=[None, rules_len])
        self.mask_code = self.weights_nonzero(self.input_code)
        self.code_embedding = tf.get_variable("code_embedding", [classnum + 10, embedding_size],
                                              dtype=tf.float32,
                                              initializer=tf.random_uniform_initializer(-math.sqrt(3), math.sqrt(3)))
        em_code = tf.nn.embedding_lookup(self.code_embedding, self.input_code)
        # my input------------- input_code ------------end -------------



        # Encoder
        # NL Reader
        with tf.variable_scope("NL_reader", reuse=None):
            nl_conv = self.transf(em_NL, self.mask_nl, 3, True, embedding_size, em_char)


        # my encoder --------------test-info reader------start---------------
        with tf.variable_scope("test_info_reader", reuse=None):
            test_info_conv = self.test_info_transf(em_test_info, self.mask_test_info, 3, True, embedding_size, em_input_test_info_char)
        # my encoder --------------test-info reader------end---------------

        # my encoder --------------code reader------start---------------
        state = em_code
        state_slice = slice(0, 2)
        update_shape = tf.shape(state)[state_slice]
        halting_probability = tf.zeros(update_shape, name="halting_probability")
        ...
        for i in range(int(self.max_steps - 1)):
            with tf.variable_scope("code_reader" + str(i), reuse=None):
                (state,  step, halting_probability, remainders, n_updates,
                 previous_state, em_Rule_Type, nl_conv,test_info_conv, em_code) = \
                    self.code_reader(state,  step, halting_probability, remainders, n_updates,
                                    previous_state, em_Rule_Type, nl_conv,test_info_conv, em_code)
        code_conv = state
        # my encoder --------------code reader------end---------------
        ...
        with tf.variable_scope("RL_conv", reuse=None):
            ...
            for i in range(int(self.max_steps - 1)):
                with tf.variable_scope("AST_READER" + str(i), reuse=None):
                    (state, self.embedding_size, step, halting_probability, remainders, n_updates,
                     previous_state, em_Rule_Type, nl_conv, em_Rule_List,code_conv,test_info_conv) = \
                        self.ast_reader(state, self.embedding_size, step, halting_probability, remainders, n_updates,
                                        previous_state, em_Rule_Type, nl_conv, em_Rule_List,code_conv,test_info_conv)
            Decoder = state
            ...
            # add fangshi
            # state = state + code_conv
            # nl_conv = nl_conv + test_info_conv

            for i in range(int(self.max_steps - 1)):
                with tf.variable_scope("DECODER", reuse=None):
                    with tf.variable_scope("QUERY_DECODER" + str(i), reuse=None):
                        (f_state, self.embedding_size, step, halting_probability, remainders,
                         n_updates, previous_state, state, nl_conv, em_Tree_Path,test_info_conv,code_conv) \
                            = self.my_query_decoder(f_state, self.embedding_size, step, halting_probability, remainders,
                                                 n_updates, previous_state, state, nl_conv, em_Tree_Path,test_info_conv,code_conv)

                        # add fangshi
                        # (f_state, self.embedding_size, step, halting_probability, remainders,
                        #  n_updates, previous_state, state, nl_conv, em_Tree_Path,test_info_conv,code_conv) \
                        #     = self.my_query_decoder_2(f_state, self.embedding_size, step, halting_probability, remainders,
                        #                          n_updates, previous_state, state, nl_conv, em_Tree_Path,test_info_conv,code_conv)
        ...
        self.correct_prediction = tf.cast(tf.equal(tf.argmax(self.y_result, 2), tf.argmax(self.inputY, 2)),
                                          tf.float32) * loss_mask

        #YUE
        self.res_y_result = tf.argmax(self.y_result, 2)
        self.res_inputY = tf.argmax(self.inputY, 2)
        self.res_loss_mask = loss_mask

        self.accuracy = tf.reduce_mean(
            self.correct_prediction * tf.expand_dims(rules_len / tf.reduce_sum(loss_mask, reduction_indices=[1]), -1))
        self.cross_entropy = tf.reduce_sum(tf.reduce_sum(loss_mask *
                                                         -tf.reduce_sum(self.inputY * tf.log(
                                                             tf.clip_by_value(self.y_result, 1e-10, 1.0)),
                                                                        reduction_indices=[2]),
                                                         reduction_indices=[1])) / tf.reduce_sum(loss_mask,
                                                                                                 reduction_indices=[0,
                                                                                                                    1])
