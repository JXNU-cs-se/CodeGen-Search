#-*-coding:utf-8-*-
import sys
import code_generate_test_model
from resolve_data_with_test import *
import os
...
testInfoOutputDir = 'HS-B/data/test66/'

testInfoInputDir = 'HS-B/data/test66/'
...
def J_run():
    global global_step
    in_file = open(project + mynum + "Tree_Rule.in")
    #in_file.close()
    fw = open(project + mynum +"Tree_Feature.out", "w")
...
def create_model(session, g, placeholder=""):
    # if(os.path.exists(project + "test_model_out/save1")):
    #     saver = tf.train.Saver()
    #     saver.restore(session, tf.train.latest_checkpoint(project + "test_model_out/save1"))
    #     print("load the model")
    if (os.path.exists(project + mydir + "/save1")):
        saver = tf.train.Saver()
        # saver.restore(session, tf.train.latest_checkpoint(project + "test_model_out/save1/"))
        saver.restore(session, tf.train.latest_checkpoint(project + mydir + "/save1/"))
        print("load the model")
    else:
...
#     saver = tf.train.Saver()
#     saver.save(session, "test_model_out/save" + str(number) + "/model.cpkt")

class Javaoutput:
    def __init__(self, Tree, Nl, Node, PNode , Root, TreeWithEnd,
                 FatherTree, GrandFatherTree, state):
        self.Tree = Tree #0 input_Tree
        self.Nl = Nl #1
        self.Node = Node #2
        self.PNode = PNode #3
        self.Root = [Root] #4
        ...
        self.RuleList = []
        self.DeepthList = []
        self.FatherTree = FatherTree #6 input_Father
        self.TreeWithEnd = TreeWithEnd #5
        self.GrandFatherTree = GrandFatherTree #7 input_Grand
...
def getJavaOut(Nl):
    f = open(project + mynum + "Tree_Feature.out", "r")
    lines = f.readlines()
    f.close()
    # print(lines)
    if len(lines) == 2:
...
def g_predict_beam(sess, model, batch_data,testInfo, testRuleInfo):
    batch = batch_data
    rewards = np.zeros([len(batch[1])])

    for i in range(len(rewards)):
        rewards[i] = 1
    testInfoPost = line2vec(testInfo, "testInfo", 200)
    testInfoCharPost = line2charvec(testInfo, 200, char_len)
    testRuleInfoPost = rule2classvec(testRuleInfo, 200)
    testInput = []
    testInput.append(testInfoPost)
    testInput.append(testInfoCharPost)
    testInput.append(testRuleInfoPost)
    for i in range(len(testInput)):
        testInput[i] = np.expand_dims(testInput[i], axis=0)
    y = sess.run(model.y_result, feed_dict={model.input_NL: batch[0],
                                            ...
                                            model.input_test_info: testInput[0],
                                            model.input_test_info_Char: testInput[1],
                                            model.input_code: testInput[2]
...
def getAction(sess, Code_gen_model, JavaOut,testInfo, testRuleInfo):
    ...
    list_input = []
    list_input.append(input_NL) #0
    list_input.append(input_NLChar) #1
    list_input.append(input_Tree) #2
    list_input.append(input_Father) #3
    list_input.append(input_Grand) #4
    list_input.append("") #5
    list_input.append(input_Rules) #6 model.inputrulelist,
    v1, v2 = line2rules(rules_str, length[5], father_vec, JavaOut.Nl)
    list_input.append(v1) #7 model.inputrulelistnode
    #print (v1)
    list_input.append(v2) #8 model.inputrulelistson
    #print (v2)
    global step
    ...
    #            list_input[-1][i][t] = 0
    step += 1
    list_input.append(tree_path_vec) #9 model.tree_path_vec
    #print (listu_input)
    #exit()
    ...
    list_input.append(ret) #10 model.treemask
    list_input.append(father_vec) #11 model.father_mat
    list_input.append(labels) #12 model.labels
    for i in range(len(list_input)):
        list_input[i] = np.expand_dims(list_input[i], axis=0)
    
    return g_predict_beam(sess, Code_gen_model, list_input,testInfo = testInfo, testRuleInfo = testRuleInfo)

def WriteJavaIn(JavaOut, action):
    f = open(project + mynum + "Tree_Rule.in", "w")
...
def BeamSearch(sess, Code_gen_model, Nl, N, NL_number,testInfo, testRuleInfo):
    # myRules = []
    ...
    while True:
        ...
        for JavaOut in Beam:
            ...
            if JavaOut.is_end :
                ...
                continue
            print ("-----------")
            res = getAction(sess, Code_gen_model, JavaOut,testInfo = testInfo, testRuleInfo = testRuleInfo)
        ...
        if endnum >= N:
            # print("end")
            # test_data's predict rule; number=beam search

            f = open(testInfoOutputDir + str(NL_number) + ".txt", "w")
            for JavaOut in Beam:
            ...
            # f = open(testInfoOutputDir  + str(NL_number) + "-rule.txt", "w")
            for JavaOut in Beam:
                myRuleList = deepcopy(JavaOut.RuleList)
                for i in range(len(myRuleList)):
                    if myRuleList[i] > 854:
                        myRuleList[i] = myRuleList[i] - 854 + 10000 - 1
                    else:
                        myRuleList[i] = myRuleList[i] - 1
                # f.write(JavaOut.Nl)
                # f.write('\n')
                # f.write(' '.join(list(map(str, myRuleList))))
                # f.write('\n')
            # f.close()
            break
...
def predict():
    ...
    Code_gen_model = code_generate_test_model.code_gen_model(classnum, embedding_size, conv_layernum, conv_layersize, rnn_layernum,
    ...
    with tf.Session(config=config) as sess:
        ...
        if(mynum == 'test66'):
            ...
            f = open(project + "my_out_trans_nl.txt", "r")
        elif(mynum == 'dev64'):
            f = open(project + "my_out_dev_nl.txt", "r")
        ...
        for i in range(len(lines)):
            ...
            # yue
            testInfoFilePath = testInfoInputDir + '{}-testInfoMin.txt'.format(i)
            # if(not os.path.exists(testInfoFilePath)):
            #     continue
            myf = open(testInfoFilePath, 'r')
            testInfolines = myf.readline()
            myf.close()
            testInfoline = testInfolines
            # testInfoline = ''

            testInfoRuleFilePath = testInfoInputDir + '{}-rule.txt'.format(i)
            myf = open(testInfoRuleFilePath, 'r')
            testInfoRuleline = myf.readlines()[0]
            myf.close()
            # testInfoRuleline = ''
            Nl = lines[i].strip()        
            print(Nl)
            # temp file; first token input ;
            f = open(project + mynum + "Tree_Feature.out", "w")
            f.write("root ^")       #0 self.TreeWithEnd
            f.write("\n")
            f.write("root")         #1 self.Node
            f.write("\n")
            f.write("Unknown")      #2 self.PNode
            f.write("\n")
            f.write("root\n")       #3 self.Root
            f.write("root node_gen ^ ^\n")      #4 self.Tree
            f.write("root node_gen ^ ^\n")      #5
            f.write("Unknown root ^ ^\n")       #6 self.FatherTree
            f.write("Unknown Unknown ^ ^\n")    #7 self.GrandFatherTree
            f.write("root node_gen ^\n")
            f.write("Unknown root ^ ^\n")
            f.write("Unknown Unknown ^ ^\n")
            f.close()
            BeamSearch(sess, Code_gen_model, Nl, int(sys.argv[2]), i,testInfo = testInfoline, testRuleInfo = testInfoRuleline)
