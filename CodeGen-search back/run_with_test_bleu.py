...
# from resolve_data_with_test_without_test  import *
import nltk
...
def create_model(session, g, placeholder=""):
    ...
    # if(os.path.exists(project + "model-out_my_best/save1")):
...
def save_model(session, number):
    saver = tf.train.Saver()
    # saver.save(session, project + "test_model_out/save" + str(number) + "/model.cpkt")
    saver.save(session, project + mydir + "/save" + str(number) + "/model.cpkt")

def save_model_time(session, number, card):
    saver = tf.train.Saver()
    # saver.save(session, project + "test_model_out/save_list/save" + str(number) + "_" + str(card) + "/model.cpkt")
    saver.save(session, project + mydir + "/save_list/save" + str(number) + "_" + str(card) + "/model.cpkt")
...
def g_pretrain(sess, model, batch_data):
    ...
    _, pre, a = sess.run([model.optim, model.correct_prediction, model.cross_entropy], feed_dict={model.input_NL: batch[0],
                                                  ...
                                                  model.is_train: True,
                                                  model.input_test_info: batch[19],
                                                  model.input_test_info_Char: batch[20],
                                                  model.input_code: batch[21]
...
def g_eval(sess, model, batch_data):
    ...
    state = get_state(batch_data)
    acc, pre, pre_rules, res_y_result, res_inputY, res_loss_mask = sess.run(
        [model.accuracy, model.correct_prediction, model.max_res,
         model.res_y_result, model.res_inputY, model.res_loss_mask,
    ...
    return acc, p, max_res,  res_y_result, res_inputY, res_loss_mask
...
def run():
    ...
    valid_batch, _ = batch_data(8, "dev") # read data
    best_accuracy = 0
    best_card = 0
    ...
    f = open(project + mydir + "/out.txt", "w")
    with tf.Session(config=config) as sess:
        create_model(sess, Code_gen_model, "")
        best_time = -1
        # writer = tf.summary.FileWriter('logssss', sess.graph)
        # writer.close()
            ...
            for j in tqdm(range(len(batch))):
                if i % 3 == 0 and j % 2000 == 0: #eval
                    ...
                    for k in range(len(valid_batch)):
                        ac1, loss1, max_res, res_y_result, res_inputY, res_loss_mask = g_eval(sess, Code_gen_model, valid_batch[k])
                        #
                        # ac1, loss1, _ = g_eval(sess, Code_gen_model, valid_batch[k])
                        x, y = res_loss_mask.shape
                        for i in range(x):
                            gen_code = res_y_result[i].tolist()
                            true_code = res_inputY[i].tolist()
                            count = res_loss_mask[i].tolist()
                            count = int(sum(count))

                            # bleu += nltk.translate.bleu_score.sentence_bleu(gen_code[:count], true_code[:count])
                            bleu += nltk.translate.bleu_score.sentence_bleu( [true_code[:count]]   ,  gen_code[:count])

                        res.extend(loss1)
                        ac += ac1;
                    #yue
                    bleu = bleu / len(valid_batch)
                    ...
                    # yue
                    print("current better blue: {}".format(bleu))
                    if best_blue < bleu:
                    # if best_card < card:
                        best_card = card
                        best_accuracy = ac
                        ...
                        #yue
                        best_blue = bleu
                        print("my find better blue: {}".format(best_blue))
                    # yue
                    elif bleu == best_blue:
                    # elif card == best_card:
                        if (best_accuracy < ac):
                ...
                if i % 50 == 0 and j == 0:
                    ...
                    for k in range(len(valid_batch)):
                        # ac1, loss1, _ = g_eval(sess, Code_gen_model, valid_batch[k])
                        ac1, loss1, _, _, _, _ = g_eval(sess, Code_gen_model, valid_batch[k])
