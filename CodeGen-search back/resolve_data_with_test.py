...
def line2vec (line, tp, length):
    ...
    if "tree" == tp:
        ...
        for i in range(min(len(tokens), length)):
            vec[i] = word2vec(tokens[i], "testInfo")
    else:
        for i in range(min(len(tokens), length)):
...
def read_data (file_name):
    ...
    similarDir = project + 'data/'
    if(file2number[file_name] == 0):
        similarDir += 'train529/'
    elif(file2number[file_name] == 1):
        # similarDir = project + 'treegen_model_out-best/'
        similarDir += 'dev64/'
    elif (file2number[file_name] == 2):
        similarDir += 'test66/'
    myCount = None
    errorList = []
    okList = []
    ...
    for i in tqdm(range(len(lines))):
        ...
        if t == 0 : # the first line; Natural Languages;
            ...
            if bf != lines[i] and bf != "" and len(bf.split()) < length[0] and addFlag: # length protection
            ...
            # yue :add testInfo and testInfo char and output rules
            similarFile = similarDir + '{}-testInfoMin.txt'.format(myCount)
            #yue
            myf = open(similarFile,'r')
            similarline = myf.readline()
            # similarline = ''
            myf.close()
            # print('this --------- similarline = null')

            # similarline = myf.readlines()
            # myf.close()
            # similarline = ' NextLineToken '.join(testInfolines)

            # print(similarline)

            # if("IndentationError" in similarline or "CardQuery" in similarline):
            #     addFlag = False
            #     errorList.append(myCount)
            #
            #     print("is error : {}".format(myCount))
            # print("this is {0}len(similarline) :  {1}".format(myCount, len(similarline)))
            # if(len(similarline) == 0):
            #     addFlag = False
            #     similarline = 'OK'
            #     okList.append(myCount)
            #     print("this is ok : {}".format(myCount))


            each_vec.append(line2vec(similarline, "testInfo", 200))
            # each_vec.append(line2vec(similarline, "testInfo", length[t]))
            each_vec.append(line2charvec(similarline, 200, char_len))
            # each_vec.append(line2charvec(similarline, length[t], char_len))
            similarRuleFile = similarDir + '{}-rule.txt'.format(myCount)
            myf = open(similarRuleFile, 'r')
            similarRuleline = myf.readlines()[0]
            # similarRuleline = ''
            myf.close()

            each_vec.append(rule2classvec(similarRuleline, 200))
    ...
    print("index_of_dataset is :{}".format(index_of_dataset))
    print("len trainset[index_of_dataset] is :{}".format(len(trainset[index_of_dataset])))
    # print(trainset[index_of_dataset])
    print(errorList)
    print(okList)
    return
...
def batch_data (batch_size, dataset_name): # get an acceptable data for NN;
    ...
    for i in range(len(data)):
        # yue
        data_now += [[data[i][q][t] for t in range(22)] for q in range(len(data[i]))]
    ...
    for site in range(len(all_data)):
        # yue
        ret_data += [[[all_data[site][i][t] for i in range(len(all_data[site]))] for t in range(22)]]
